		...
		if (isset_IN('ids'))
		{
			...
			if (count($ids) > 0)
			{
                ...
                foreach ($ids as $pid) {
                    ...
                    if ($did and ($p[$pid]>0))
                        $db->update('Plans',array('pmndID'=>$did, 'pmnPerc'=>$p[$pid]),'','pID=?d',array($pid));
                }
                //----------------------------
				...
				foreach ($ids as $pid) {
					$dl = $db->select('Deps LEFT JOIN Plans ON pID=dpID LEFT JOIN Users ON uID=duID', '*', 
						'dpID=?d and dState=1 and (dNPer<pNPer or pNPer=0) and dNTS<=? and dLTS<=?', array($pid, timeToStamp(), timeToStamp(time() - 10 * HS2_UNIX_MINUTE)));
